...
lowercase_full_names = [name.lower() for name in full_names]#name只是名字，没有含义，用x也OK
...
big_numbers = [n for n in numbers if n >9] #[结果 for 元素 in 列表 if 条件]
print(big_numbers)
...
result = ", ".join([name.split()[1] for name in full_names if len(name) > 11]) #[表达式 for 元素 in 列表 if 条件]#[表达式 for 元素 in 列表]
print(result)#.join()吧字符串变成列表